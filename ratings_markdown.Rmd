...
```
We can observe that most of the top athletes deals quite well regardless of the type of wind. However, some are more sensitive to tail wind, in particular Norwegians. The other extreme is when a jumper is better, when tail wind is present. We can see that *Dawid Kubacki* or *Cene Prevc* can be considered as this type of jumper.
...
```
From this chart we can see that ratings over 1700 are impressive and not many jumpers (either men or women) can achieve that level of mastery. The record so far is approximately 1950 points and is hold by *Stefan Kraft* since March 2020. These results should be taken seriously only if we consider the 2015-2021 time period, although the data are collected from 2009/2010 season. The reason is that the algorithm needs some time to rating distribution become stationary. 
