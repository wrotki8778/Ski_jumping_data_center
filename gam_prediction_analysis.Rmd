...
The results suggest that the further development of the model increased the explained variance from 53% to 57.5%. Now we will check how it projects on the predictions phase:
...
predict_comp_two <- function(code, dataset, simple_model, model){
  ...
  test_dataset['error'] = test_dataset['dist'] - test_dataset['prediction']
  kable(test_dataset[,c('name','round','dist','linear_prediction','linear_error','prediction','error')])
}
...
test_dataset = test_dataset %>% 
  mutate_at(vars(gam_t, epsilon_t, prediction, error),
            .funs = funs(round(. * hill_size,1)))
test_dataset = merge(test_dataset, all_names,how='left',by.x='codex.x',by.y='codex')
kable(test_dataset[,c('name','round','dist','gam_t','epsilon_t','prediction','error')])
```
