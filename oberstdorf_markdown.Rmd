...
```{r}
plot(actual_results[,c(1,2,9,11,14,16)])
round(cor(actual_results[,c(1,2,9,11,14,16)],use = 'complete.obs'),2)
```
...
ggplot(subset(actual_results[,c('dist','id')], dist>0)) +
  geom_density(aes(x = dist),fill="white", alpha = 0.6)+labs(title='Normalized density of distances')
ggplot(subset(actual_results[,c('speed','id')], speed>85)) +
  geom_density(aes(x = speed),fill="white", alpha = 0.6)+labs(title='Normalized density of speed')
...
ggplot(subset(actual_results[,c('speed','dist','id')], speed>85 & dist>0 & id %in% obe_men)) +
  geom_density_2d_filled(aes(x = speed, y= dist))+labs(title='Normalized bivariate distribution for men') + xlim(86,100) + ylim(80,140)
obe_men = subset(obe_ids, gender=='Men')$id
ggplot(subset(actual_results[,c('speed','dist','id')], speed>85 & dist>0 & !(id %in% obe_men))) +
  geom_density_2d_filled(aes(x = speed, y= dist))+labs(title='Normalized bivariate distribution for women') + xlim(86,100) + ylim(80,140)
```
...
server_gate <- function(input, output) {
  ...
  ggplot(gate_results()) +
  geom_histogram(aes(x = speed, color = gender),fill = "white", alpha = 0.6) + labs(title = N) + xlim(85,97.5)
})
...
server_dist <- function(input, output) {
  ...
  N = paste('Results from : ', input$years, ' Gender:', input$gender)
  ggplot(gate_results()) +
